...
This project is based on [KernelGYM](https://github.com/hkust-nlp/KernelGYM), with modifications for our own use.

## 开发与迭代记录

迭代记录详见文档：[KERNELGYM_OPTIMIZATION_NOTES.md](KERNELGYM_OPTIMIZATION_NOTES.md)
- 启动环境均通过 [scripts/auto_configure.sh](scripts/auto_configure.sh)配置
- auto_configure会生成 .env.hostname 配置环境，相关log和kernel保存路径也会加以区分，方便代码复用
- 配置CPU worker数，可启动CPU worker编译
- kernel默认读写/tmp，可配置 KERNELGYM_TMP_ROOT=/dev/shm 更改读写目录
- 默认编译 backend 为 manual_ninja，搭配 MANUAL_NINJA_OBJECT_CACHE=true，缓存部分固定的.h文件编译结果
...
1. Recommended: let KernelGYM auto-generate a machine-specific `.env.<hostname>` on first startup.
2. Manual: create `.env.<hostname>` yourself with explicit values, or point `ENV_FILE` to a custom file.
...
By default:
- launcher/runtime env lives in `.env.<hostname>`
- process-output logs live under `logs/<hostname>/`
- Python logging files live under `kernelgym/py_logs/<hostname>/`
...
# 2) Auto-configure machine-specific env (or skip; start script can do this automatically)
...
You can also use the debug helper script, which writes process logs and evaluation results into `logs/debug/`:

```bash
./start_all_with_monitor_debug.sh
```
...
Notes:
- If `.env.<hostname>` does not exist, `start_all_with_monitor.sh` triggers `scripts/auto_configure.sh`.
- If Redis is configured as local (`localhost`/`127.0.0.1`) and not running, the startup script launches Redis automatically.
- `start_all_with_monitor.sh` redirects process stdout/stderr into `logs/<hostname>/`.
- `start_all_with_monitor_debug.sh` writes redirected process logs and eval results into `logs/debug/` without hostname sharding.
- Python logging files are written to `kernelgym/py_logs/<hostname>/`.
- Persisted evaluation results are written to `logs/<hostname>/eval_results.jsonl`.
...
1. Create a machine-specific env file pointing to the main node:
```bash
...
| Script | Description |
|--------|-------------|
| [scripts/auto_configure.sh](scripts/auto_configure.sh) | Auto-generates machine-specific env configuration with detected IP, available ports, and GPU devices |
| [start_all_with_monitor.sh](start_all_with_monitor.sh) | Single-node deployment: starts Redis, API server, worker monitor, and GPU workers, and captures process stdout/stderr under `logs/<hostname>/` |
| [start_all_with_monitor_debug.sh](start_all_with_monitor_debug.sh) | Debug startup wrapper that writes process logs and eval results into `logs/debug/` |
...
- `bash scripts/auto_configure.sh --force` to overwrite the current env target
- `bash scripts/auto_configure.sh --use-indexed-ports` to use `PORT0`, `PORT1`, ... as candidates

Useful startup options:
- `bash start_all_with_monitor.sh --log-dir logs/custom-name` to override the process-output/results directory
- `bash start_all_with_monitor.sh --eval-results-path logs/custom-name/eval_results.jsonl` to override the persisted evaluation-results file
- `bash start_all_with_monitor.sh --py-log-dir py_logs/custom-name` to override the Python logging directory under `kernelgym/`
