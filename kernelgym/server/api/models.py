...
class EvaluationRequest(BaseModel):
    ...
    split_compile_and_execute: bool = Field(
        default=False,
        description=(
            "Split a CUDA-agent/shared-library kernel evaluation into an internal "
            "compile stage and a GPU execution stage. Triton-style tasks should "
            "leave this false."
        ),
    )
    pure_compile_task: bool = Field(
        default=False,
        description=(
            "Treat request as compile-only and skip execution/correctness/performance."
        ),
    )
    enable_compile_artifact_cache: bool = Field(
        default=False,
        description=(
            "Reuse compiled kernel artifacts for matching compile+test requests "
            "using CACHE_INDEX/COMPILE_ARTIFACT_CACHE_INDEX=memory or redis."
        ),
    )
