...
from .utils import get_gpu_info, get_system_health, get_system_metrics, format_timestamp
...
def _get_cpu_env_info() -> Dict[str, Any]:
    """Return CPU resource info exported by .env, with lightweight fallbacks."""
    return {
        "cpu_arch": settings.cpu_arch or os.uname().machine,
        "cpu_model": settings.cpu_model,
        "cpu_threads": settings.cpu_threads or (os.cpu_count() or 1),
        "cpu_physical_cores": settings.cpu_physical_cores,
        "cpu_sockets": settings.cpu_sockets or 1,
        "cpu_numa_nodes": settings.cpu_numa_nodes or 1,
        "memory_total_mb": settings.memory_total_mb,
        "cpu_compile_workers": settings.cpu_compile_workers,
    }


def _candidate_server_urls(host: str, port: int) -> List[str]:
    """Build a small set of likely reachable server URLs for startup logs."""
    urls: List[str] = []

    def _add(url_host: str) -> None:
        if not url_host:
            return
    ...
    if normalized_host in wildcard_hosts:
        _add("127.0.0.1")
        _add("localhost")
        try:
...
async def lifespan(app: FastAPI):
    ...
    logger.info("API server URLs: %s", ", ".join(_candidate_server_urls(settings.api_host, settings.api_port)))
...
async def get_workers_status(
    ...
    """Get workers status."""
    try:
        return await task_mgr.get_workers_status()
...
async def get_resources_status(
    ...
    """Get CPU, GPU, and worker status without changing legacy worker API shape."""
    try:
        return {
            "cpu": _get_cpu_env_info(),
            "gpu": await get_gpu_info(),
            "workers": await task_mgr.get_workers_status(),
        }
...
async def register_worker(
    ...
    try:
        logger.info(
            f"/worker/register received: worker_id={worker_id}, device={device}, "
            f"worker_type={worker_type}, node_id={node_id}, hostname={hostname}"
        )
        success = await task_manager.register_worker(
            worker_id,
            device,
            node_id=node_id,
            hostname=hostname,
...
async def worker_heartbeat(
    ...
    try:
        logger.info(
            f"/worker/heartbeat received: worker_id={worker_id}, device={device}, "
            f"worker_type={worker_type}, node_id={node_id}, hostname={hostname}"
        )
...
if __name__ == "__main__":
    ...
    setup_logging("api")
    logger.info("Launching API server on: %s", ", ".join(_candidate_server_urls(settings.api_host, settings.api_port)))
