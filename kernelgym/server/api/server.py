...
# Create FastAPI app
app = FastAPI(title="KernelGym", description="GPU Kernel Evaluation Service", version="1.0.0", lifespan=lifespan)
...
async def allocate_node_id(hostname: str, node_name: Optional[str] = None) -> Dict[str, Any]:
...
async def get_task_manager() -> TaskManager:
    ...
    if task_manager is None:
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Task manager not available")
    return task_manager
...
async def get_redis_client() -> redis.Redis:
    ...
    if redis_client is None:
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Redis client not available")
    return redis_client


def _result_status(payload: Dict[str, Any]) -> TaskStatus:
    return task_status_from_result_payload(payload)
...
async def evaluate_kernel(
    request: EvaluationRequest, background_tasks: BackgroundTasks, task_mgr: TaskManager = Depends(get_task_manager)
):
    ...
    try:
        ...
        for task_request in request.tasks:
            try:
                ...
                if status_value in (TaskStatus.FAILED, TaskStatus.TIMEOUT):
                    failed_tasks += 1
...
async def get_workers_status(task_mgr: TaskManager = Depends(get_task_manager)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to get workers status: {str(e)}"
        )
...
async def register_worker(
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to register worker")
    except Exception as e:
...
async def unregister_worker(worker_id: str, task_manager: TaskManager = Depends(get_task_manager)) -> Dict[str, Any]:
...
async def worker_heartbeat(
    ...
    try:
        ...
        await task_manager.update_worker_heartbeat(worker_id, node_id=node_id, hostname=hostname)
        lb_keys = list(task_manager.worker_load_balancer.available_workers.keys())
...
async def evict_from_lb(worker_id: str, task_manager: TaskManager = Depends(get_task_manager)) -> Dict[str, Any]:
