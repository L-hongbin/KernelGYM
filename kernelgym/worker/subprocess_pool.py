    ...
    try:
        ...
        while True:
            try:
                ...
                result_requires_recycle = result.get("worker_exiting") is True
                must_recycle = result_requires_recycle or tasks_processed + 1 >= max_tasks_per_worker
                if result_requires_recycle:
                    # A structured Sanitizer failure means the CUDA context may
                    # already be poisoned. Publish without another CUDA call,
                    # then retain ancestry until the parent contains and reaps us.
