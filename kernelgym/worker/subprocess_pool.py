...
def _execute_task_in_worker(
    ...
    try:
        task_stage = str(task_data.get("task_stage") or "").lower()
        if task_stage == "compile" or bool(task_data.get("pure_compile_task")):
            from kernelgym.worker.compile_stage import run_compile_only_task

            return run_compile_only_task(task_data, worker_id, str(device))
