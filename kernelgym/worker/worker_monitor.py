...
from kernelgym.config.settings import PROJECT_ROOT, settings
...
class WorkerMonitor:
    ...
    def __init__(self, redis_client: redis.Redis, persistent: bool = False):
        ...
        self.startup_grace_sec = max(10, getattr(settings, "worker_monitor_startup_grace_sec", 90))
        logger.info(
            "Worker monitor configured with heartbeat_timeout=%ss, monitor_interval=%ss, restart_cooldown=%ss, startup_grace=%ss",
        ...
        try:
            ...
            for wid in missing_ids:
                if wid in self.restart_in_progress:
                    continue
                if await self._should_delay_missing_heartbeat_restart(wid):
                    continue
    ...
    async def _worker_has_recent_heartbeat(self, worker_id: str) -> bool:
        worker_data = await self.redis.hgetall(f"{KEY_PREFIX}:worker:{worker_id}")
        if not worker_data:
            return False
        if worker_data.get(b"cuda_error_shutdown") == b"true":
            return False
        last_heartbeat_raw = worker_data.get(b"last_heartbeat")
        ...
        try:
            ...
            return False
        return datetime.now() - last_heartbeat <= timedelta(seconds=self.heartbeat_timeout)

    async def _worker_process_info(self, worker_id: str) -> tuple[int, Optional[datetime]]:
        proc_info = await self.redis.hgetall(f"{KEY_PREFIX}:worker_process:{worker_id}")
        if not proc_info:
            return 0, None
        try:
            pid_raw = proc_info.get(b"pid")
            ...
            pid = 0
        start_time_raw = proc_info.get(b"start_time")
        start_time = None
        if start_time_raw:
            try:
    ...
    async def _should_delay_missing_heartbeat_restart(self, worker_id: str) -> bool:
        pid, start_time = await self._worker_process_info(worker_id)
        if not await self._is_pid_alive(pid):
            return False
        ...
        if age < self.startup_grace_sec:
            logger.info(
                "Worker %s has no heartbeat yet but process is alive and within startup grace (%ss < %ss); skip restart",
                worker_id,
                round(age, 1),
                self.startup_grace_sec,
            )
    ...
    async def _should_skip_restart(self, worker_id: str, reason: str) -> bool:
        if await self._worker_has_recent_heartbeat(worker_id):
            logger.info("Skipping stale restart for %s: worker heartbeat is healthy again", worker_id)
            return True
        if "heartbeat" in reason.lower() and await self._should_delay_missing_heartbeat_restart(worker_id):
            logger.info("Skipping restart for %s: worker is still in startup grace", worker_id)
            return True
        return False
    ...
    async def _restart_loop(self):
        ...
        while self.running:
            try:
                ...
                if await self._should_skip_restart(worker_id, reason):
...
"""
    ...
    async def _restart_worker(self, worker_id: str, device: str) -> bool:
        ...
        try:
            ...
            # Reset GPU device to clear CUDA error state. CPU compile workers
            # have no CUDA state to reset.
            ...
            logs_dir = Path(os.environ.get("LOG_DIR", f"logs/{os.environ.get('HOSTNAME', 'local')}"))
            if not logs_dir.is_absolute():
                logs_dir = PROJECT_ROOT.parent / logs_dir
