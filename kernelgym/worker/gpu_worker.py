...
from kernelgym.worker.compile_stage import run_compile_only_task, sanitize_compile_artifact
...
class GPUWorker:
    ...
    def __init__(self, worker_id: str, device: str, redis_client: redis.Redis):
        ...
        self.stop_task_grace_sec = max(0, getattr(settings, "worker_stop_task_grace_sec", 20))
        self.split_compile_mode = str(
            getattr(settings, "worker_split_compile_mode", "background_compile")
        ).strip().lower()
        if self.split_compile_mode not in {"legacy", "background_compile"}:
            raise ValueError(
                "Invalid WORKER_SPLIT_COMPILE_MODE: "
                f"{self.split_compile_mode}. Expected legacy or background_compile."
            )
        ...
        # GPU device setup (主进程不使用CUDA，只存储device_id). CPU workers
        # reuse the same registration/queue loop but only poll CPU resource queues.
        if device.startswith("cuda:"):
            self.device_id = int(device.split(":")[1])
            self.worker_resource = "gpu"
        elif device.startswith("cpu:"):
            self.device_id = None
            self.worker_resource = "cpu"
        else:
            raise ValueError(f"Invalid device format: {device}, expected 'cuda:N' or 'cpu:N'")
        ...
        self._background_split_compile_enabled = self.split_compile_mode == "background_compile"
    ...
    async def start(self):
        ...
        try:
            ...
            try:
                ...
                await self.redis.hset(
                    ...
                    mapping={
                        ...
                        "current_task": "",
                        "tasks_processed": "0",
                        "worker_type": self.worker_resource,
                    }
                )
            ...
            if self.worker_resource == "gpu":
                ...
                # ============================================================
                await self._initialize_gpu_worker_pools()
            else:
                logger.info(
                    f"Starting CPU worker {self.worker_id}; CUDA initialization "
                    "and GPU subprocess pool are disabled"
                )
            ...
            logger.info(
                "Worker %s ready; execution lock=serial",
                self.worker_id,
            )
    ...
    async def stop(self):
        ...
        if current_task:
            try:
                await asyncio.wait_for(self._task_done_event.wait(), timeout=self.stop_task_grace_sec)
                logger.info(
                    "Worker %s waited for in-flight task %s to finish during shutdown",
                    self.worker_id,
                    current_task,
                )
                ...
                if self.current_task == current_task:
                    try:
                        await self.task_manager.fail_task(current_task, "Worker shutdown")
                    except Exception:
                        pass

        if self._background_tasks:
            pending_background_tasks = [task for task in self._background_tasks if not task.done()]
            if pending_background_tasks:
                background_timeout = max(
                    self.stop_task_grace_sec,
                    self.stop_task_grace_sec * len(pending_background_tasks),
                )
                try:
                    await asyncio.wait_for(
                        asyncio.gather(*pending_background_tasks, return_exceptions=True),
        ...
        await self._shutdown_gpu_worker_pools()
    ...
    async def _initialize_gpu_worker_pools(self):
        """Initialize subprocess pools for GPU-backed execution."""
        logger.info(
    ...
    def _should_background_compile_task(self, task_data: Dict[str, Any]) -> bool:
        return (
            self.worker_resource == "gpu"
            and self._background_split_compile_enabled
            and self._is_compile_only_task(task_data)
            and bool(task_data.get("execute_after_compile_on_gpu"))
            and not bool(task_data.get("pure_compile_task"))
        )

    def _track_background_task(self, task: asyncio.Task, task_id: str) -> None:
        ...
        def _cleanup(done_task: asyncio.Task) -> None:
            ...
            try:
                ...
                if exc is not None:
                    logger.error(
                        "Background compile task %s failed unexpectedly: %s",
                        task_id,
                        exc,
                    )
            except asyncio.CancelledError:
                logger.warning("Background compile task %s cancelled", task_id)
            except Exception as exc:
                logger.error(
                    "Failed to inspect background compile task %s: %s",
                    task_id,
                    exc,
                )
    ...
    async def _enqueue_background_compile_task(self, task_data: Dict[str, Any]) -> None:
        await self._background_compile_semaphore.acquire()
        if not self.running or getattr(self, "_stopping", False):
            self._background_compile_semaphore.release()
            return
        task_id = task_data["task_id"]
        background_task = asyncio.create_task(self._process_background_compile_task(task_data))
        self._track_background_task(background_task, task_id)

    async def _process_background_compile_task(self, task_data: Dict[str, Any]):
        """Process a split compile task in the background."""
        try:
            await self._process_task(task_data, background=True)
        finally:
            self._background_compile_semaphore.release()

    async def _process_task(self, task_data: Dict[str, Any], background: bool = False):
        ...
        try:
            ...
            error_message = "Task processing failed: Worker shutdown"
            logger.warning(
                "Worker %s cancelled task %s during shutdown",
                self.worker_id,
                task_id,
            )
            failed_result = self._build_failed_result(
                task_data,
                error_message,
                classify_error(error_message, "runtime"),
            )
            failed_result["processing_time"] = (datetime.now() - start_time).total_seconds()
            try:
    ...
    def _build_failed_result(
        ...
        metadata = {
            "error": error_message,
            "device_info": get_cuda_device_info(task_data.get("device")),
        }
    ...
    async def _run_compile_stage(self, task_data: Dict[str, Any], timeout_sec: int) -> Dict[str, Any]:
        return await asyncio.wait_for(
            asyncio.to_thread(self._run_compile_only_task, task_data),
            timeout=timeout_sec,
        )

    async def _run_execute_after_compile(
        self,
        task_data: Dict[str, Any],
        compile_artifact: Dict[str, Any],
        timeout_sec: int,
    ) -> Dict[str, Any]:
        inline_task = dict(task_data)
        inline_task["task_stage"] = "execute"
        inline_task["required_resource"] = "gpu"
        inline_task["compile_artifact"] = compile_artifact
        inline_task["pure_compile_task"] = False
        for key, value in (task_data.get("post_compile_execution_options") or {}).items():
            inline_task[key] = value
        ...
        if result_data.get("success", False):
            inline_result = result_data.get("result", {})
            inline_result = dict(inline_result)
            metadata = dict(inline_result.get("metadata") or {})
            metadata["inline_gpu_execute_completed"] = True
            metadata["inline_compile_worker_id"] = self.worker_id
            metadata["inline_compile_worker_device"] = self.device
            metadata["compile_artifact"] = self._sanitize_compile_artifact(compile_artifact)
            metadata.pop("_internal_compile_artifact", None)
            inline_result["metadata"] = metadata
            result_data["result"] = inline_result
        return result_data

    def _is_compile_only_task(self, task_data: Dict[str, Any]) -> bool:
        return str(task_data.get("task_stage") or "").lower() == "compile" or bool(
            task_data.get("pure_compile_task")
        )

    def _run_compile_only_task(self, task_data: Dict[str, Any]) -> Dict[str, Any]:
        return run_compile_only_task(task_data, self.worker_id, self.device)

    def _sanitize_compile_artifact(self, artifact: Dict[str, Any]) -> Dict[str, Any]:
        return sanitize_compile_artifact(artifact)

    def _run_cpu_toolkit_task(self, task_data: Dict[str, Any]) -> Dict[str, Any]:
        """Run an explicitly CPU-routed task without initializing CUDA in the worker."""
        try:
            from kernelgym.backend import get_backend
            from kernelgym.toolkit import get_toolkit

            toolkit = get_toolkit(task_data["toolkit"])
    ...
    async def start(self):
        ...
        try:
            ...
            if worker_class in {"legacy", "gpuworker"}:
                ...
                worker_cls = BackgroundCompileGPUWorker
            else:
                raise ValueError(
            ...
            for device in settings.gpu_devices:
                ...
                worker = worker_cls(worker_id, device_name, self.redis_client)
