...
def resolve_env_file() -> str:
    ...
    hostname_value = os.environ.get("HOSTNAME")
    ...
    if hostname_value:
        machine_env_file = PROJECT_ROOT / f".env.{hostname_value}"
        if machine_env_file.exists():
            return str(machine_env_file)

    return str(PROJECT_ROOT / ".env")


def get_hostname_value() -> str:
    hostname_value = os.environ.get("HOSTNAME")
    if hostname_value:
    ...
    try:
        ...
        hostname_value = ""
    return hostname_value or "local"


def default_log_dir() -> str:
    return f"py_logs/{get_hostname_value()}"
...
class Settings(BaseSettings):
    ...
    cpu_arch: str = Field(default="", env="CPU_ARCH")
    cpu_model: str = Field(default="", env="CPU_MODEL")
    cpu_threads: int = Field(default=0, env="CPU_THREADS")
    cpu_physical_cores: int = Field(default=0, env="CPU_PHYSICAL_CORES")
    cpu_sockets: int = Field(default=0, env="CPU_SOCKETS")
    cpu_numa_nodes: int = Field(default=0, env="CPU_NUMA_NODES")
    memory_total_mb: int = Field(default=0, env="MEMORY_TOTAL_MB")
    cpu_compile_workers: int = Field(default=0, env="CPU_COMPILE_WORKERS")
    ...
    eval_results_path: str = Field(
        default=str(KERNELBENCH_ROOT / f"logs/{get_hostname_value()}" / "eval_results.jsonl"),
    ...
    worker_monitor_startup_grace_sec: int = Field(default=90, env="WORKER_MONITOR_STARTUP_GRACE_SEC")
    worker_use_blocking_task_poll: bool = Field(
        default=True,
        env="WORKER_USE_BLOCKING_TASK_POLL",
        description="Use blocking Redis queue pop for worker task polling. Disable to restore legacy busy-poll behavior.",
    )
    gpu_worker_class: str = Field(
        default="legacy",
        env="GPU_WORKER_CLASS",
        description="GPU worker implementation to launch: legacy or background_compile.",
    )
    worker_split_compile_mode: str = Field(
        default="background_compile",
        env="WORKER_SPLIT_COMPILE_MODE",
        description="Split compile scheduling mode for GPU workers: legacy or background_compile.",
    )
    worker_background_compile_limit: int = Field(
        default=2,
        env="WORKER_BACKGROUND_COMPILE_LIMIT",
        description="Max number of split compile tasks a GPU worker may compile concurrently in the background.",
    )
    worker_compile_pool_size: int = Field(
        default=2,
        env="WORKER_COMPILE_POOL_SIZE",
        description="Number of subprocess workers reserved for compile tasks in BackgroundCompileGPUWorker.",
    )
    worker_compile_max_tasks_per_worker: int = Field(
        default=10,
        env="WORKER_COMPILE_MAX_TASKS_PER_WORKER",
        description="Max tasks a compile subprocess handles before restart in BackgroundCompileGPUWorker.",
    )
    worker_task_poll_block_timeout_sec: int = Field(
        default=1,
        env="WORKER_TASK_POLL_BLOCK_TIMEOUT_SEC",
        description="Blocking timeout, in seconds, when workers wait for the next task queue entry.",
    )
    ...
    worker_stop_task_grace_sec: int = Field(default=20, env="WORKER_STOP_TASK_GRACE_SEC")
    gpu_workers_poll_cpu_tasks: bool = Field(
        default=True,
        env="GPU_WORKERS_POLL_CPU_TASKS",
        description="Allow GPU workers to pick CPU/compile tasks only after GPU queues are empty.",
    )
    ...
    log_dir: str = Field(default=default_log_dir(), env="PY_LOG_DIR")
