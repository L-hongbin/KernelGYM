...
def main() -> int:
    ...
    # Do not resolve these paths: venv ``bin/python`` is commonly a symlink to
    # the system interpreter, and resolving it bypasses the adjacent
    # ``pyvenv.cfg`` that selects the environment being measured.
    environments = [
        ("shared", Path(args.shared_python).expanduser().absolute()),
        ("local", Path(args.local_python).expanduser().absolute()),
    ]
        ...
        json.dumps(
            {
                "config": {
                    **vars(args),
                    "shared_python": str(environments[0][1]),
                    "local_python": str(environments[1][1]),
                },
                "records": records,
                "summaries": summaries,
            },
