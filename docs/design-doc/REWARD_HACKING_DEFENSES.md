...
Unknown receivers such as `x.sqrt()` pass the static framework-compute check under the B+ policy and are adjudicated by the fail-closed candidate-forward ATen gate. Explicit `.half()`/`.float16()` and `.to(torch.float16)` remain statically forbidden for FP32 tasks because the runtime ATen compatibility allowlist intentionally permits tensor casts and therefore cannot prove that precision was preserved. The selected candidate entry point's `__init__` may construct standard `torch.nn` activation and normalization module objects; the same constructors remain forbidden in other classes and methods, and directly resolved PyTorch/ATen compute plus every other static safety rule remains active inside `__init__`. Constructor-derived values are tracked through aliases, destructuring, literal containers, and literal `getattr`/`setattr`; invoking one or passing one to a non-storage call during construction is rejected. Calls made while constructing the candidate are outside the current ATen profiling window, so constructor-stage unknown-receiver compute remains an accepted gap.
