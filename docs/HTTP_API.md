...
| `enable_compute_sanitizer` | `false` by default; `null` uses server `ENABLE_COMPUTE_SANITIZER`. Sanitizer is independent of `return_detail_correctness`; when enabled, a fresh child process is launched after a candidate correctness runtime error or output mismatch. |
...
When `enable_correctness_input_perturbations=true`, at least four correctness trials are run. Direct floating-point outputs of `torch.rand` use `original`, `x3`, `x0.01`, and negation; direct floating-point outputs of `torch.randn` use `original`, `x3`, `x0.01`, and absolute value. Integer, boolean, scalar, and unrecognized inputs are unchanged. If the reference raises or produces NaN/Inf for a non-original perturbation, that perturbation is recorded in `metadata.correctness_reference_skipped_perturbations` and excluded from the correctness denominator. Numerical kernel mismatches always retain the legacy `max_difference`, `avg_difference`, `correctness_atol`, and `correctness_rtol` metadata fields. The remaining diagnostics in this paragraph require `return_detail_correctness=true`: `element_correctness_curve` reports the element-count-weighted percentage satisfying `abs(candidate - reference) <= multiplier * (atol + rtol * abs(reference))` at `1x`, `2x`, `4x`, `8x`, and `16x` the base tolerance, formatted with two decimal places such as `99.80%`. The former standalone `element_correctness` metadata field is no longer returned; consumers read the base result from `element_correctness_curve[i]["1x"]`. Once any curve point reaches `100.00%`, later points are neither compared nor included in the returned curve or issue text. `nan_count` and `inf_count` count non-finite candidate-output elements. `mismatch_localization[i].element` contains the first and last mismatch in output traversal order plus `top_3`, the three most severe coordinates ordered by normalized error; each coordinate carries an `output_path` for nested outputs. These diagnostics are lists aligned with numerical mismatch trials. The base percentage and curve are included in the `correctness_issue` text, which also identifies the failed perturbation. Performance and memory measurements continue to use the original input distribution. `correctness_failed_input_perturbation` is returned only when input perturbations are enabled, for numerical/structure mismatches and runtime failures; otherwise it is omitted, and Sanitizer replay defaults to original inputs.

With `return_detail_correctness=true`, rank-2-or-higher tensor outputs additionally return `mismatch_localization[i].output_space`. Its `batch_correctness`, `row_correctness`, and `tile_correctness` values are verifier fractions from 0 to 1. A rank-3 `B x M x N` output is split into whole `B` slices, `(B,M)` rows over `N`, and `32 x 32` tiles over the last two axes; partial tail tiles use their actual size. Each batch, row, or tile summary contains `mismatch_count`, at most 8 entries in `mismatches`, and `mismatches_truncated`; `output_space` also includes inclusive mismatch bounds such as `"N": [128, 143]`. Tile ranges use half-open `[start, end]` coordinates. Rank-1 outputs return only the `element` section and omit `output_space`.
...
| `compilation_error_detail` | Returned only when compiler diagnostic excerpts were extracted; otherwise omitted. Maps each stable category to `{count, truncated, errors}`: `count` is the full deduplicated count for that category, `errors` contains the first at most 8 unique excerpts in source order, and `truncated` indicates omitted entries. Each excerpt preserves its error line and immediately associated source line; caret-only lines are omitted and locations are simplified for the response. No separate summary is returned. Original compilation error text remains available separately. |
| `compilation_error` | Original compilation error text (existing path simplification still applies). This field is unchanged by summarization: when parsed details exist, top-level `error_message` uses category/count summaries and the first diagnostic line (up to 300 characters); otherwise it includes the full error text without a new length cap. |
| `compile_artifact` | Public description of the compiled artifact. Nested `compiled` and `error` are omitted; use top-level `compiled` for compile/load success and `metadata.compilation_error` for failures. Internal artifacts retain both fields for execution and cache handling. |
...
| `element_correctness_curve` | Detailed mode only: per-mismatching-trial correctness curves at `1x`, `2x`, `4x`, `8x`, and `16x` the base tolerance; read the base element correctness from `[i]["1x"]`. |
| `nan_count`, `inf_count` | Detailed mode only: per-mismatching-trial counts of non-finite candidate-output elements. |
| `mismatch_localization` | Detailed mode only: per-mismatching-trial `element` coordinates plus optional rank-2-or-higher `output_space` batch/row/tile verifier summaries, bounded mismatch lists, and axis bounds. Replaces `mismatch_coordinate` and `output_space_localization`. |
| `correctness_diagnosis` | Detailed mode only: high-confidence pattern diagnosis with `category`, `confidence`, and `evidence`. Human-readable diagnosis is appended to `correctness_issue` and therefore included in `error_message`; no separate `text` field is returned. Omitted when Sanitizer runs or the mismatch pattern is not sufficiently typical. |
