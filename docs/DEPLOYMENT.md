...
- Nsight Compute collection is disabled by default. When enabled, it uses `/usr/local/cuda-12.9/bin/ncu`; the runtime validator then checks the executable and version, and deployed workers must have permission to access NVIDIA GPU performance counters. The compact metric set includes L1 throughput (`l1tex__throughput.avg.pct_of_peak_sustained_active`), L1 sector hit rate (`l1tex__t_sector_hit_rate.pct`), L2 throughput (`lts__throughput.avg.pct_of_peak_sustained_elapsed`), and L2 sector hit rate (`lts__t_sector_hit_rate.pct`). It does not currently collect request/sector counts or read/write byte totals.
- Set `ENABLE_NCU=true` globally, use request field `enable_ncu=true` for one evaluation, or pass `null` to inherit the server setting.
- Detailed correctness is request-gated by `return_detail_correctness`, which defaults to `false`; the default path retains legacy correctness metadata and skips the added mismatch calculations and Runtime Sanitizer.
- Runtime Sanitizer uses `/usr/local/cuda-12.9/bin/compute-sanitizer`; deployment validation fails fast if the deployment-level tool setting is enabled but the executable is missing.
- It runs after a candidate correctness forward raises or completes with an output mismatch and regenerates that trial's input in an isolated process. Under `error_based`, a specifically classified runtime error runs only its matching check; an ambiguous runtime error tries `memcheck`, `synccheck`, `racecheck`, then `initcheck`; an output mismatch tries `racecheck`, `initcheck`, `memcheck`, then `synccheck`. These staged paths stop after the first detected issue. Explicit `full` runs all four checks in canonical order without early stopping. Per-check limits are 20/15/30/20 seconds respectively, capped by a 60-second whole-diagnostic budget; mismatch-triggered feedback is omitted unless an issue is detected.
- Set both request fields `return_detail_correctness=true` and `enable_compute_sanitizer=true` to enable isolated trials. `enable_compute_sanitizer=null` may inherit `ENABLE_COMPUTE_SANITIZER`, but the request-level detailed-correctness gate is always required.
